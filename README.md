...
A second function, `roadmap`, is optional in the same way, and needed only to work on the public roadmap. It receives a webhook from Linear on every issue change, and mirrors the issues explicitly marked public into the `roadmap_issue` table. It takes two more variables in the same file:

```env
LINEAR_WEBHOOK_SECRET=<YOUR_WEBHOOK_SIGNING_SECRET>
LINEAR_PUBLIC_LABEL_ID=<YOUR_PUBLIC_LABEL_ID>
```

The secret is generated by Linear when the webhook is created, under the API settings of the workspace, and the label id is that of the label which opts an issue into the roadmap, read from the [GraphQL API](https://linear.app/developers/graphql). Neither is optional to the function: with either one missing, every delivery is answered with a `500`, since a webhook with no secret would have to accept unverified writes, and one with no label id could not tell a public issue from a private one.

The webhook itself points at `/functions/v1/roadmap/webhook`, and is subscribed to `Issue` events alone. A local server is not reachable from Linear, so deliveries are best exercised by signing a body yourself and posting it to the local endpoint.
