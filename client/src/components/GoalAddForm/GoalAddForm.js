...
const GoalAddForm = (setSubmitting) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      closePopup();
      addMessage(
        `New goal was added! You should now be able to select it as an option.`,
        "success",
        8000,
      );
    } catch (error) {
      ...
      if (error.code === "23505") {
        addMessage(
          "A goal already exists with either the same color name or color!",
          "error",
          10000,
        );
      } else {
        addMessage(
          "There was a problem adding the goal. If the issue consists, the system may be experiencing an outage.",
          "error",
          13000,
        );
      }
