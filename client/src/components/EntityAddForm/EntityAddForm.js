...
const EntitiyAddForm = (setSubmitting, refreshSelectDataFunc) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addMessage(
        `New ${entity} was added! You should now be able to select it as an option.`,
        "success",
        8000,
      );
    } catch (error) {
      ...
      if (error.code === "23505") {
        ...
        if (entity === "platform") {
          errorMsg =
            "A platform exists with the same name or abbreviation, or both!";
        } else {
        ...
        addMessage(
          "There was a problem during the entity creation process. If the issue consists, the system may be experiencing an outage.",
          "error",
          15000,
        );
      }
