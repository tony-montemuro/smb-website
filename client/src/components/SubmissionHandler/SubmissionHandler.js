...
const SubmissionHandler = (isUnapproved) => {
  ...
  const fetchSubmissions = async (game) => {
    ...
    if (
      (isUnapproved && game.unapproved === 0) ||
      (!isUnapproved && game.reported === 0)
    ) {
      setSubmissions([]);
      return;
    }
    ...
    try {
      const query = isUnapproved
        ? getUnapprovedByGame(game.abb)
        : getReportedByGame(game.abb);
      const submissions = await query;
      setSubmissions(submissions);
    } catch (error) {
      addMessage(
        "Submission data failed to load. If refreshing the page does not work, the system may be experiencing an outage.",
        "error",
        10000,
      );
    }
  ...
  const isClickable = (submission) => {
    return !(
      ...
      (submission.profile.id === user.profile.id ||
        submission.report.creator.id === user.profile.id)
    );
  };
