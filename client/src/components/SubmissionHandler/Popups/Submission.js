...
const Submission = (
  ...
  const handleChange = (e) => {
    ...
    setForm({
      ...
      error: Object.keys(form.error).includes(id)
        ? { ...form.error, [id]: null }
        : { ...form.error },
    });
  };
  ...
  const handleToggle = () => {
    setForm({
      ...
      values: {
        ...form.values,
        comment: clearToggle ? submission.comment : "",
      },
    });
  ...
  const handleCloseAndRemove = () => {
    setSubmissions((submissions) =>
      submissions.filter((row) => row !== submission),
    );
    dispatchGames({
      type: isUnapproved ? "decrementUnapproved" : "decrementReported",
      value: game.abb,
    });
    handleClose();
  };
  ...
  const isFormUnchanged = () => {
    return (
      ...
      (!submission.version ||
        form.values.version === submission.version.id.toString()) &&
  ...
  const approveSubmission = async () => {
    setSubmitting(true);
    try {
      const query = isUnapproved
        ? insertApproval(submission.id, user.profile.id)
        : deleteReport(submission.report.report_date);
      await query;
      handleCloseAndRemove();
      addMessage("Submission was successfully approved!", "success", 5000);
    } catch (error) {
      addMessage(
        "There was a problem approving this submission.",
        "error",
        7000,
      );
    } finally {
  ...
  const updateAndApproveSubmission = async () => {
    ...
    try {
      ...
      payload.submitted_at = getDateOfSubmission(
      ...
      handleCloseAndRemove();
      addMessage(
        "Submission was successfully updated and approved!",
        "success",
        7000,
      );
    } catch (error) {
      if (error.message === "approve") {
        ...
        addMessage(
          "There was a problem updating this submission.",
          "error",
          7000,
        );
      }
  ...
  const onRejectClick = async (e) => {
    ...
    try {
      ...
      if (error.message === "delete") {
        addMessage(
          "There was a problem deleting this submission.",
          "error",
          7000,
        );
      } else {
        addMessage(
          "The submission successfully was rejected, but the notification system failed to notify the user.",
          "error",
          10000,
        );
      }
