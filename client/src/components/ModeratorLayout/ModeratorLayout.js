...
const ModeratorLayout = () => {
  ...
  const reducer = (state, action) => {
    ...
    switch (type) {
      ...
      case "decrementUnapproved":
        ...
        decremented = {
          ...state[index],
          unapproved: state[index].unapproved - 1,
        };
        return [
          ...state.slice(0, index),
          decremented,
  ...
  const updateLayout = async () => {
    try {
      const filteredGames = !user.profile.administrator
        ? user.profile.game.map((game) => game.abb)
        : [];
      const games = await getUnapprovedCounts(filteredGames);
      dispatchGames({ type: "all", value: games });
    } catch (error) {
      addMessage(
        "Submission metadata failed to load. If refreshing the page does not work, the system may be experiencing an outage.",
        "error",
        10000,
      );
