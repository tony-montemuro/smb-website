...
const UserSearch = () => {
  ...
  const searchUsers = (userInput, gamesPerPage, pageNum) => {
    if (
      !badSearch ||
      userInput.length < badSearch.length ||
      !userInput.startsWith(badSearch)
    ) {
  ...
  const updateResults = async (userInput, usersPerPage, pageNum) => {
    ...
    try {
      const { profiles, count } = await searchForProfiles(
        userInput,
        start,
        end,
      );
      ...
      if (profiles.length === 0)
        setBadSearch((badSearch) => (badSearch ? badSearch : userInput));
    } catch (error) {
