...
const CategoryAddForm = (setSubmitting) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      addMessage(
        "Form not validated, please see error messages.",
        "error",
        8000,
      );
      return;
    ...
    try {
      ...
      closePopup();
      addMessage(
        `New category was added! You should now be able to select it as an option.`,
        "success",
        8000,
      );
    } catch (error) {
      ...
      if (error.code === "23505") {
        addMessage(
          "A category exists with the same name and practice mode style!",
          "error",
          10000,
        );
      } else {
        addMessage(
          "There was a problem adding the category. If the issue consists, the system may be experiencing an outage.",
          "error",
          13000,
        );
      }
