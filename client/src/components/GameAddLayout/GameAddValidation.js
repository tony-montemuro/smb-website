...
const GameAddValidation = () => {
  ...
  const validateCategories = (categories, modes) => {
    ...
    if (hasInvalid) {
      errorMsg +=
        "One or more categories have invalid values, contact TonySMB for help. ";
    }
    if (hasMissingModes) {
  ...
  const validateModes = (modes, categories, levels) => {
    ...
    modes.forEach((obj) => {
      ...
      if (!categories.map((c) => c.category).includes(category)) {
        errorMsg +=
          "Mode belongs to an invalid category, contact TonySMB for help. ";
      }
  ...
  const validateLevels = (levels, categories, modes) => {
    ...
    levels.forEach((level) => {
      ...
      if (!categories.map((c) => c.category).includes(category)) {
        errorMsg +=
          "Level assigned to invalid category, contact TonySMB for help. ";
      }

      // next, validate mode is valid
      if (!modes.map((m) => m.id).includes(mode.id)) {
        errorMsg +=
          "Level assigned to invalid mode, contact TonySMB for help. ";
      }
      ...
      if (chart_type === "score") {
        ...
        if (["both", "time"].includes(ascending)) {
          errorMsg +=
            "Ascend Time cannot be checked when Chart Type is Score. ";
        }
      } else {
        if (timer_type === "") {
          errorMsg +=
            "Timer Type is required when Chart Type is Time or Both. ";
        }
    ...
    const duplicateMessage =
      "Duplicate Name + Goal combination within this category. ";
  ...
  const validateMetadata = (metadata) => {
    ...
    if (metadata.custom) {
      error.min_date = validateMinDate(
        metadata.min_date,
