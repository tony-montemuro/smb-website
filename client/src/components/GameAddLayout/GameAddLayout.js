...
const GameAddLayout = (
  ...
  const restoreUnlockedPagesState = () => {
    const localUnlockedPages = JSON.parse(
      localStorage.getItem(unlockedPagesKey),
    );
  ...
  const unlockNextPage = () => {
    ...
    if (page.number === numPages) {
      ...
      addMessage(
        `Information validated! Please proceed to the ${pageNames[numPages]} section.`,
        "success",
        8000,
      );
    } else {
  ...
  const fetchEntitiesData = async () => {
    try {
      ...
      addMessage(
        "Entities data failed to load. If reloading the page does not work, the system may be experiencing an outage.",
        "error",
        15000,
      );
    }
  ...
  const getTransformedCategories = () => {
    return [...Object.values(appData.categories)].sort(
      (b, a) => b.practice - a.practice,
    );
  };
  ...
  const fetchStructureData = async () => {
    try {
      ...
      addMessage(
        "Structure data failed to load. If reloading the page does not work, the system may be experiencing an outage.",
        "error",
        15000,
      );
    }
  ...
  const updateStructureCategories = async () => {
    setStructureData((prevState) => ({
      ...prevState,
      categories: getTransformedCategories(),
    }));
  };
