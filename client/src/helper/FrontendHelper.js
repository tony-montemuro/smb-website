...
const FrontendHelper = () => {
  ...
  const secondsToHours = (record, type, decimalPlaces = 2) => {
    if (type === "time") {
      ...
      let minutes = Math.floor((time % 3600) / 60).toLocaleString("en-US", {
      ...
      let decimals = Math.round(
        ((record % 60) - seconds) * multiplier,
  ...
  const getTimeAgo = (timestamp) => {
    ...
    const formattedTimeAgo = formatDistanceToNowStrict(submissionDate, {
      addSuffix: true,
    });
  ...
  const timerType2TimeUnit = (timerType) => {
    ...
    if (["min", "min_sec", "min_sec_csec", "min_sec_msec"].includes(timerType))
      return "minute";
    if (
      [
        "hour",
