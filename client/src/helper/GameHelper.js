const GameHelper = () => {
  ...
  const getCategoryTypes = (game, categoryName) => {
    ...
    if (!game.mode) {
      const category = game.categories.find(
        (category) => category.abb === categoryName,
      );
    ...
    for (const mode of game.mode) {
      // for each mode
      if (mode.category === categoryName) {
        for (const level of mode.level) {
          // for each level
  ...
  const fetchLevelFromGame = (game, levelName, category, type) => {
    for (const mode of game.mode) {
      // for each mode in the game object
      if (mode.category === category) {
        for (const level of mode.level) {
          // for each level in the mode object
          if (
            level.name === levelName &&
            (level.chart_type === type || level.chart_type === "both")
          ) {
            return level;
          }
        }
  ...
  const getDecimals = (levels) => {
    return levels.some(
      (level) => level.timer_type !== null && level.timer_type.endsWith("msec"),
    )
      ? 3
      : 2;
  };
