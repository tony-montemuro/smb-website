...
const StructureForm = (chartDefaults, setChartDefaults) => {
  ...
  const getCategoryDataWithName = (categoryData) => {
    return {
      ...categoryData,
      name: structureData.categories.find(
        (c) => c.abb === categoryData.category,
      ).name,
    };
  };
  ...
  const updateCategory = (state, data) => {
    ...
    const oldCategoryName = stateValues.category.find(
      (c) => c.id === data.id,
    ).category;
    if (!data.category) {
      // find children of category
      const children = {};
      children.mode = stateValues.mode.filter(
        (mode) => mode.category === oldCategoryName,
      );
      children.level = stateValues.level.filter(
        (level) => level.category === oldCategoryName,
      );
      ...
      if (
        (children.mode.length > 0 || children.level.length > 0) &&
      ...
      updated.level = updated.level.filter(
        (level) => level.category !== oldCategoryName,
      );
      updated.mode = updated.mode.filter(
        (mode) => mode.category !== oldCategoryName,
      );
    } else {
      ...
      const cascade = (arg) =>
        arg.category === oldCategoryName
          ? { ...arg, category: data.category }
          : arg;
  ...
  const insertMode = (state, data) => {
    const { elements: updatedModes, errors: updatedModeErrors } =
  ...
  const updateMode = (state, data) => {
    ...
    updated.level = updated.level.map((level) =>
      level.mode.id === data.id
  ...
  const deleteMode = (state, id) => {
    const updatedValues = { ...state.values },
      updatedErrors = { ...state.error };
    const childrenLevels = state.values.level.filter(
      (level) => level.mode.id === id,
    );
    let warning =
      "Deleting this mode will also delete it's charts. Are you sure you want to do this?";
    ...
    updatedValues.level = updatedValues.level.filter(
      (level) => level.mode.id !== id,
    );
  ...
  const insertLevel = (state, data) => {
    const { elements: updatedLevels, errors: updatedLevelErrors } =
  ...
  const reducer = (state, action) => {
    ...
    const dispatchType = Object.values(dispatchTypes).find(
      (t) => t.name === type,
    );
    if (dispatchType) {
      let updatedState;
  ...
  const isDuplicate = (data, entityName) => {
    ...
    // false otherwise.
    const result = form.values[entityName].some(
      (v) => v[entityName] === data[entityName],
    );
    if (result) {
  ...
  const handleModeInsert = (category, id = null) => {
    ...
    if (id === null) {
      let categoryIndex = form.values.category.findIndex(
        (c) => c.id === category.id,
      );

      while (categoryIndex >= 0 && id === null) {
        const categoryName = form.values.category[categoryIndex].category;
        const categoryModes = form.values.mode.filter(
    ...
    if (id === null) {
      ...
      while (categoryIndex >= 0 && id === null) {
        ...
        while (modeIndex >= minModeIndex && id === null) {
          ...
          const levels = form.values.level.filter(
            (l) => l.category === categoryName && l.mode.id === modeId,
          );
          if (levels.length > 0) {
            id = levels.at(-1).id + 1;
          } else {
  ...
  const handleLevelDelete = (id) =>
    dispatchForm({ type: dispatchTypes.DELETE_LEVEL.name, data: id });
  ...
  const handleChartDefaultsChange = (e) => {
    ...
    if (id.startsWith("ascending")) {
      value = getAscendingValue(
        id.split("_")[1],
        checked,
        chartDefaults.ascending,
      );
      id = "ascending";
    }
