...
const StructureForm = (formData, setFormData) => {
    ...
    // POSTCONDITIONS (1 possible outcome):
    // a copy of data is returned, including the name of the category, which we can grab from the `formData.categories` object
    const getCategoryDataWithName = categoryData => {
        return { ...categoryData, name: formData.categories.find(c => c.abb === categoryData.category).name }
    };
