...
const StructureForm = (setFormData) => {
    ...
    // FUNCTION 28: validateStructure - function that ensures the the user-inputted game structure is valid
    // PRECONDITIONS (1 parameter):
    ...
    const validateStructure = e => {
        ...
        form.values.category.forEach(obj => {
            const { id, category } = obj;
            if (!categories[category]) {
                error.category[id] = "Invalid category!";
            }
        });
        if (error.category.length === 0) error.category = undefined;
        ...
        form.values.mode.forEach(obj => {
            ...
            let errorMsg = "";
            if (!categories[category]) {
                errorMsg += "Mode belongs to an invalid category. Perhaps there has been a data desynchronization?";
            }
            if (name.includes("?")) {
