...
const EntitiesForm = () => {
  ...
  const { entitiesData, keys, unlockNextPage } = useContext(GameAddContext);
  ...
  // FUNCTION 3: getDataWithName - function that returns form data with the name of the entity included
  // PRECONDITIONS (2 parameters):
  // 1.) data: an object generated by the user's form selection: contains the form id (id), and entity id ([entityName])
  // 2.) entityName: a string, corresponding to an entity name
  // POSTCONDITIONS (2 possible outcome):
  // if entityName = "moderator", return data
  // otherwise, a copy of data is returned, including the name of the entity, which we can grab from the `entitesData` object
  const getDataWithName = (data, entityName) => {
    if (entityName === "moderator") {
      return data;
    }
    return {
      ...data,
      name: entitiesData[entityName].find((e) => e.id === data[entityName])[
        `${entityName}_name`
      ],
    };
  };

  // FUNCTION 4: reducer - function that executes each time the user attempts to update the form reducer hook
  // PRECONDITIONS (2 parameters):
  ...
  const reducer = (state, action) => {
    ...
    switch (type) {
      ...
      case "updateValues": {
        ...
        if (data[name]) {
          updated.push(getDataWithName(data, name));
  ...
  // FUNCTION 5: populateForm - function that executes when the EntitiesForm component mounts
  // PRECONDITIONS: NONE
  ...
  const handleUpdate = (value, id, entityName, name) => {
