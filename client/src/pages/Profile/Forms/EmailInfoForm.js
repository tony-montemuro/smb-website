...
const EmailInfoForm = () => {
  ...
  const handleEmailUpdate = async (e) => {
    ...
    if (error) {
      ...
      addMessage(
        "There was a problem with the email you provided.",
        "error",
        7000,
      );
      return;
    ...
    try {
      ...
      addMessage(
        `An email has been sent to both your current address, as well as ${email.name}.`,
        "success",
        7000,
      );
      ...
      addMessage(
        "There was a problem sending the reset emails. If refreshing the page does not work, the email system may be experiencing an outage.",
        "error",
        11000,
      );
