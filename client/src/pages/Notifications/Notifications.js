...
const Notifications = () => {
  ...
  const updateNotifications = async (notifsPerPage) => {
    ...
    try {
      ...
      dispatchNotifications({
        field: "selected",
        payload: {
          data: notifications.selected[pageNum]
            ? notifications.selected[pageNum]
            : [],
          pageNum: pageNum,
        },
      });
      ...
      addMessage(
        "Notifications failed to load. If refreshing the page does not work, the system may be experiencing an outage.",
        "error",
        10000,
      );
    }
  ...
  const areAllNotifsSelected = () => {
    ...
    const notifDates = notifications.all.map(
      (notification) => notification.notif_date,
    );
  ...
  const toggleSelection = (id) => {
    ...
    if (idIndex > -1) {
      dispatchNotifications({
        ...
        payload: {
          data: [
            ...notifications.selected[pageNum].slice(0, idIndex),
            ...notifications.selected[pageNum].slice(idIndex + 1),
          ],
          pageNum: pageNum,
        },
      });
    ...
    // otherwise, add it to the selected object
      ...
      dispatchNotifications({
        field: "selected",
        payload: {
          data: [...notifications.selected[pageNum], id].sort((a, b) =>
  ...
  const removeSelected = async () => {
    ...
    Object.keys(notifications.selected).forEach((key) => {
      const pagePromises = notifications.selected[key].map((notif_date) =>
        deleteNotification(notif_date),
      );
    ...
    try {
      ...
      addMessage(
        "One or more notifications failed to delete. Refresh the page and try again.",
        "error",
        10000,
      );
    } finally {
