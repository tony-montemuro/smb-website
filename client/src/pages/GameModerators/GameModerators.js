...
const GameModerators = () => {
  ...
  const removeModerator = async (moderator, closePopup) => {
    ...
    try {
      ...
      addMessage(
        "There was an error trying to remove this moderator. Try refreshing the page.",
        "error",
        8000,
      );
    } finally {
  ...
  const addModerator = async (moderator, closePopup) => {
    ...
    try {
      ...
      if (error.code === "23505") {
        addMessage(
          "This user is already a moderator for this game!",
          "error",
          5000,
        );
      } else {
        addMessage(
          "There was an error trying to add this moderator. Try refreshing the page.",
          "error",
          8000,
        );
      }
