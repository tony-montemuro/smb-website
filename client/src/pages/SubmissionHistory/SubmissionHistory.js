...
const SubmissionHistory = () => {
  ...
  const fetchProfile = async (profileId) => {
    try {
      ...
      addMessage(
        "Failed to fetch some or all of Submission History data. If refreshing the page does not work, the system may be experiencing an outage.",
        "error",
        12000,
      );
      return undefined;
  ...
  const fetchSubmissions = async () => {
    ...
    try {
      ...
      const submissions = await getChartSubmissionsByProfile(
        abb,
        category,
        levelName,
        type,
