...
function Update({ level, updateBoard, submitting, setSubmitting }) {
  ...
  const {
    ...
    isFormUnchanged,
    handleSubmit,
  } = UpdateLogic(level, setSubmitting);
        ...
        <form onSubmit={(e) => handleSubmit(e, submissions, updateBoard)}>
          <div className={styles.formWrapper}>
            ...
            {form.values.approved && (
              <p>
                <b>Note:</b> Since this submission has been approved by a moderator, any updates
                will revoke it&apos;s approval.
              </p>
            )}
              ...
              slotProps={{
                textField: {
                  color: form.error.submitted_at
                    ? "error"
                    : form.values.submitted_at !== dateB2F(form.submission.submitted_at)
                      ? "success"
                      : "primary",
                  fullWidth: true,
            ...
            {game.version.length > 0 && (
              <TextField
                color={
                  parseInt(form.values.version) !== form.submission.version ? "success" : "primary"
                }
                fullWidth
                helperText={
                  parseInt(form.values.version) !== form.submission.version ? updateFieldText : null
                }
                id="version"
                label="Game Version"
                select
