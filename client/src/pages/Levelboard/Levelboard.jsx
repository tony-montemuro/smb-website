  ...
  return level ? (
    <div className={styles.levelboard}>
      ...
      <Container>
        ...
        <div className={`center ${styles.title}`}>
          ...
          {board.adjacent && board.adjacent.prev ? (
            <Link
              to={addAllExistingSearchParams(
                `/games/${abb}/${category}/${type}/${board.adjacent.prev}`,
              )}
            >
          ...
          {board.adjacent && board.adjacent.next ? (
            <Link
              to={addAllExistingSearchParams(
                `/games/${abb}/${category}/${type}/${board.adjacent.next}`,
              )}
            >
            ...
            <table>
              ...
              <thead>
                <tr>
                  ...
                  {isPracticeMode && (
                    <th id={styles.medalsIcon}>
                      <BananaIcon title="Medals" />
                    </th>
                  )}
                  ...
                  <TableContent
                    ...
                    emptyMessage={
                      board.all.length > 0
                        ...
                        : "There have been no submissions to this chart."
                    }
                    numCols={TABLE_WIDTH}
                  >
                    {board.filtered
                      .slice((pageNum - 1) * SUBMISSIONS_PER_TABLE, pageNum * SUBMISSIONS_PER_TABLE)
                      .map((submission) => {
                        return (
                          <LevelboardRow
      ...
      <Container title="Recent Submissions" largeTitle>
        <RecentSubmissionsTable numSubmissions={NUM_RECENT} searchParams={searchParams} />
      </Container>
    </div>
  ) : (
    <Loading />
