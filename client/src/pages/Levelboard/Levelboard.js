...
const Levelboard = () => {
  ...
  const insertPositionToLevelboard = (submissions) => {
    ...
    submissions.forEach((submission, index) => {
      ...
      if (
        ...
        submissions[index + 1].record !== submission.record
      ) {
        posCount = trueCount;
      }
    });
    ...
    const filtered = submissions.filter((submission) => {
      return (
        ...
        isBefore(
          ...
          getInclusiveDate(filters.endDate),
        ) &&
  ...
  const setupBoard = async (filters) => {
    ...
    try {
      ...
      const all = await getChartSubmissions(
        abb,
        category,
        levelName,
        type,
      ...
      addMessage(
        "Failed to fetch / update chart data. If refreshing the page does not work, the system may be experiencing an outage.",
        "error",
        10000,
      );
    } finally {
  ...
  const getChartTypes = (level) => {
    if (level.chart_type === "both") {
      return [
        { data: "score", renderedData: "Score" },
        { data: "time", renderedData: "Time" },
      ];
    } else {
      return [
        { data: level.chart_type, renderedData: capitalize(level.chart_type) },
      ];
    }
  };
