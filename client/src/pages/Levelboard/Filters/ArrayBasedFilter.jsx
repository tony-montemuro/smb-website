    ...
    <div>
      ...
      <div className={styles.header}>
        ...
        {hasFilterChanged(selectedFilter.propertyName) && (
          <button type="button" onClick={() => handleFilterReset(selectedFilter.propertyName)}>
            Reset
          </button>
        )}
      </div>
      ...
      <div className={styles.btns}>
        ...
        <button
          type="button"
          className={
            filters[selectedFilter.propertyName].length === selectedFilter.items.length
              ? styles.selected
              : ""
          }
          onClick={() =>
            handleFilterChangeAll(
              selectedFilter.items.map((item) => item.id),
        ...
        {selectedFilter.items.map((item) => {
          return (
            <button
              type="button"
              className={
                filters[selectedFilter.propertyName].includes(item.id) ? styles.selected : ""
              }
              onClick={() =>
                handleFilterChange(
                  item.id,
