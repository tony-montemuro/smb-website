...
const Insert = (level, setSubmitting) => {
  ...
  const recordToSeconds = (
    ...
    let time = 0;
    const hour = parseInt(hourField),
      minute = parseInt(minuteField),
      second = parseInt(secondField),
      centisecond = parseInt(centisecondField),
      millisecond = parseInt(millisecondField);
  ...
  const getSubmissionsFromForm = (formVals) => {
    ...
    if (both) {
      ...
      submissions.push({
        ...
        record:
          type === "time"
            ? recordToSeconds(hour, minute, second, centisecond, millisecond)
            : submission.record,
      });
    }
  ...
  const close = async (updateBoard, successMsg, timer) => {
    ...
    if (
      submissionVersion !== "" &&
      submissionVersion !== parseInt(version?.id)
    ) {
    ...
    if (r1.status === "fulfilled") {
      ...
      if (r2.status === "fulfilled") {
        ...
        if (type === "time") {
          dispatchForm({
            field: "values",
            value: { record: defaultVals.record },
          });
        } else {
          const { hour, minute, second, centisecond, millisecond } =
            defaultVals;
          dispatchForm({
            field: "values",
        ...
        addMessage(
          `Your ${otherType} submission succesfully added, but there was a problem adding your ${type} submission. Try again or refresh the page.`,
          "error",
          15000,
        );
      } else {
        addMessage(
          "There was a problem adding both of your submissions. Try refreshing the page.",
          "error",
          10000,
        );
      }
  ...
  const handleSubmit = async (e, timerType, updateBoard) => {
    ...
    if (type === "time" || (type === "score" && form.values.both)) {
      ...
      error.centisecond = validateCentisecond(
        form.values.centisecond,
        timerType,
      );
      error.millisecond = validateMillisecond(
        form.values.millisecond,
        timerType,
      );
      allBlank =
        !form.values.hour &&
    ...
    const submission = submissions.shift(),
      otherSubmission = submissions.shift();
    ...
    if (!form.values.both) {
      if (r1.status === "fulfilled") {
        ...
        addMessage(
          "There was a problem adding your submission. Try refreshing the page.",
          "error",
          8000,
        );
      }
