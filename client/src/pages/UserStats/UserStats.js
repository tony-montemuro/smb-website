...
const UserStats = (decimalPlaces, setVersion) => {
  ...
  const fetchUserStats = async (game, profileId, category, type, version) => {
    ...
    try {
      ...
      const [allTotals, liveTotals, medals, allRankings, liveRankings] =
        await Promise.all(promises);

      // next, we need to extract the total / medal objects belonginging to the user defined by `profileId`
      const allTotal = allTotals.find(
        (total) => total.profile.id === profileId,
      );
      const liveTotal = liveTotals.find(
        (total) => total.profile.id === profileId,
      );
      ...
      addMessage(
        "Failed to fetch user data. If refreshing the page does not work, the system may be experiencing an outage.",
        "error",
        10000,
      );
    }
