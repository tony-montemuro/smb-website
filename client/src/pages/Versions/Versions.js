...
const Versions = () => {
  ...
  const getStructure = useCallback(
    async (game) => {
      try {
        ...
        addMessage(
          "There was a problem loading the charts for this game. If this error persists, the system may be experiencing an outage.",
          "error",
          15000,
        );
      }
    },
    [queryStructureByGame, setGame, addMessage],
  ); // this prevents function from ever being redefined: none of these should change
  ...
  const switchGame = async (newGame) => {
    ...
    if (game) {
      ...
      if (
        unsaved &&
        !window.confirm(
          "Are you sure you want to switch games? Your progress will be lost!",
        )
      ) {
        return;
      }
  ...
  const queryGames = async () => {
    try {
      ...
      addMessage(
        "There was an error fetching the moderator data. If refreshing the page does not work, the system may be experiencing an outage.",
        "error",
        10000,
      );
    }
  ...
  const handleVersionsChange = (e) => {
    ...
    const updatedVersions = versions.map((version) =>
      version.sequence === sequence ? { ...version, version: value } : version,
    );
  ...
  const handleNewVersionSubmit = (version) => {
    ...
    setVersions([
      ...
      {
        version: version,
        sequence: versions.at(-1).sequence + 1,
      },
    ]);
  };
  ...
  const onVersionCheck = useCallback(
    (checked, version, categoryName, levelName) => {
      const gameCopy = { ...game };
      const category = gameCopy.structure.find(
  ...
  const toggleAllPerCategory = useCallback(
    (checked, version, category) => {
      const gameCopy = { ...game };
  ...
  const toggleAllPerMode = useCallback(
  ...
  const buildVersions = useCallback(() => {
    ...
    if (malformedVersions.length > 0) {
      addMessage(
        "A fatal error has occured. Please contact TonySMB about this error, and refresh the page. Apologies for the inconvenience.",
        "error",
        15000,
      );
      return undefined;
  ...
  const handleStructureSubmit = useCallback(
    async (e, setIsSubmitting) => {
      ...
      try {
        ...
        if (versions.some((version) => version.charts.length > 0)) {
          successMsg +=
            ", and all relevant submissions' versions were updated.";
        } else {
          successMsg += ".";
        }
        ...
        if (isVersionsUpdateSuccess) {
          addMessage(
            `New version(s) successfully added to ${game.name}, but there was a problem resetting the page. Please refresh the page.`,
            "error",
            15000,
          );
        } else {
          addMessage(
            `There was a problem adding the new version(s): ${error.message}. Consider refreshing the page.`,
            "error",
            20000,
          );
        }
    ...
    [buildVersions, game, addMessage, addVersions, resetAll],
  ); // should only be redefined when versions OR game states are updated
