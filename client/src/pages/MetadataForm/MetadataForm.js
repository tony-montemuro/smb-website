...
const MetadataForm = () => {
  ...
  const handleDateChange = (e, id) => {
    ...
    if (e && !isNaN(e.$D)) {
      ...
      if (id === "release_date") {
        ...
        const minDate = form.values.min_date
          ? new Date(form.values.min_date).getTime()
          : null;

        // if game is not custom, OR min date is after release date
        if (
          !form.values.custom ||
          (releaseDate > 0 && minDate > 0 && minDate > releaseDate)
        ) {
          value.min_date = date;
        }
      }
  ...
  const validateAndUpdate = (e) => {
    ...
    if (!Object.values(error).some((e) => e !== undefined)) {
      ...
      addMessage(
        "Form not validated, please check error messages.",
        "error",
        8000,
      );
    }
  ...
  const onUserRowClick = (profile) => {
    if (profile.id !== form.values.creator?.id) {
      ...
      dispatchForm({
        field: "values",
        value: { creator: { country, id, username } },
      });
  ...
  const refreshUserSearch = () =>
    setTriggerUserSearch((triggerUserSearch) => !triggerUserSearch);
