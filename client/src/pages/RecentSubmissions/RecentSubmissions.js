...
const RecentSubmissions = () => {
  ...
  /* ===== REDUCERS ===== */
  const [filtersData, dispatchFiltersData] = useReducer(
    reducer,
    defaultFiltersData,
  );
  ...
  const fetchGames = async (searchParams) => {
    ...
    if (gameParams.length > 0) {
      try {
        ...
        games.forEach((game) => {
          const version = gameParams.find(
            (g) => g.abb === game.abb && g.version,
          )?.version;
          game.version = version ?? "";
        });
      } catch (error) {
        addMessage(
          "One or more filters has broken due loading failures.",
          "error",
          7000,
        );
        games = null;
