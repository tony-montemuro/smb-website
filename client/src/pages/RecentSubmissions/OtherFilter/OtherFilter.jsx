        ...
        <form onSubmit={() => closePopupAndUpdate(searchParams, setSearchParams)}>
          <div className={styles.filters}>
            ...
            <div className={styles.title}>
              ...
              {filters.category.length !== defaultFilters.category.length && (
                <button type="button" onClick={updateCategoryFilterAll}>
                  Reset
                </button>
              )}
            </div>
                ...
                {categoriesList.map((category) => {
                  return (
                    <button
                      ...
                      className={
                        filters.category.length === 0 || filters.category.includes(category.abb)
                          ? styles.selected
                          : ""
                      }
                    >
          ...
          <div id={styles.right} className={styles.btns}>
            <button
              type="button"
              className="cancel"
              onClick={() => resetFiltersAll(defaultFilters)}
            >
              Reset Filters
            </button>
