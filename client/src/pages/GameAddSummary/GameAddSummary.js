...
const GameAddSummary = (setError, setSubmitting) => {
  ...
  const { validateEntities, validateMetadata, validateStructure } =
    GameAddValidation();
  ...
  const createGame = async (e, metadata, entities, structure, assets) => {
    ...
    try {
      ...
      resetForm();
      addMessage(
        `${game.name} was successfully added! Double check that everything looks good, and make a post about the new game!`,
        "success",
        12000,
      );
    } catch (error) {
      addMessage(
        `There was a problem creating the game. If the issue persists, please contact TonySMB. Error returned by server: '${error.message}'`,
        "error",
        30000,
      );
    } finally {
