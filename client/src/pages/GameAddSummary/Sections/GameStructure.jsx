...
function GameStructure({ structure, error }) {
  ...
  return (
    <div className={styles.section}>
      ...
      <span>
        <em>
          <strong>Note: </strong>IDs are stored on the backend, but are typically invisible to
          users. You may notice that some IDs are skipped; this is totally normal.
        </em>
      </span>
      <hr />
      ...
      <div key={`${id}_${name}`} className={styles.mode}>
        <Errorable error={errorMessage} renderMessage>
          <span className={styles.modeName}>
            <strong>Mode: {levelB2F(name)}</strong>
          </span>
        </Errorable>
...
function Levels({ structure, category, mode, error }) {
  /* ===== VARIABLES ===== */
  const levels = structure.level.filter(
    (level) => level.category === category && level.mode.name === mode,
  );
        ...
        <tbody>
          {levels.map((level) => {
            ...
            return (
              <tr
                key={`${id}_${name}`}
                className={errorMessage && styles.levelError}
                title={errorMessage}
              >
