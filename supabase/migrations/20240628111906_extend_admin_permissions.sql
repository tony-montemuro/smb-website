...
-- Let's switch from integers to sequences for "entity" primary keys, & add character constraints to fields
ALTER TABLE monkey
...
ALTER TABLE monkey
ALTER monkey_name TYPE VARCHAR(20);

ALTER TABLE monkey RENAME CONSTRAINT temp_pkey TO monkey_pkey;

ALTER TABLE monkey RENAME CONSTRAINT temp_monkey_name_key TO monkey_name_key;

ALTER TABLE monkey
ADD CONSTRAINT monkey_name_constraint
CHECK (length(monkey_name) > 0);
...
ALTER TABLE platform
ALTER platform_name TYPE VARCHAR(30);

ALTER TABLE platform
ALTER platform_abb TYPE VARCHAR(10);

ALTER TABLE platform
ADD CONSTRAINT platform_name_constraint
CHECK (length(platform_name) > 0);

ALTER TABLE platform
ADD CONSTRAINT platform_abb_constraint
CHECK (length(platform_abb) > 0);
...
ALTER TABLE region
ADD CONSTRAINT region_name_constraint
CHECK (length(region_name) > 0);

ALTER TABLE region
ALTER region_name TYPE VARCHAR(10);
...
ALTER TABLE rule
ADD CONSTRAINT rule_name_constraint
